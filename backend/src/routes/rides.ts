...
router.post("/start", requireAuth, async (req, res) => {
  ...
  if (!scooter.isActive) return res.status(400).json({ error: "Scooter not active" });
  if (userActive) return res.status(400).json({ error: "User already has an active ride" });
  if (scooterActive) return res.status(400).json({ error: "Scooter is in use" });
